    ...
    <div class="col-span-12 flex flex-col gap-3 mx-auto w-max h-max my-auto">
      ...
      <div class="flex bg-white p-5 rounded shadow-lg flex-col items-center">
        <form action="/register" method="POST" class="flex flex-col gap-5">
          ...
          <input class="bg-white border border-black border-opacity-25 py-3 rounded px-4" type="text" name="username" placeholder="{{ __('username') }}" required>

          <button class="bg-indigo-800 p-3 px-6 text-white rounded mt-5">{{ __('register') }}</button>
        </form>
      </div>
    </div>
  </div>
